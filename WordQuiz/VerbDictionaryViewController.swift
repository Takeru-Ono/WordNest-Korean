//
//  VerbDictionaryViewController.swift
//  WordQuiz
//
//  Created by Takeru Ono on 2024/11/18.
//
import UIKit
import AVFoundation

class VerbDictionaryViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func setupUI() {
        ...
        tableView.register(VerbTableViewCell.self, forCellReuseIdentifier: "WordCell")
    ...
    func loadQuestions() {
        let categories = CSVFileManager.loadVerbCategories()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "WordCell", for: indexPath) as? VerbTableViewCell else {
            fatalError("Failed to dequeue WordTableViewCell")
        }
...
class VerbTableViewCell: UITableViewCell {
    ...
    func configure(with question: QuestionProtocol, isExpanded: Bool) {
        ...
        meaningLabel.text = question.correctAnswer.isEmpty ? "意味がありません" : question.correctAnswer
