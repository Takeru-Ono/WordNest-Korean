//
//  FavoriteViewControllerKR.swift
//  WordQuiz
//
...
struct FavoriteQuestion: Codable {
    var text: String // 質問テキスト
    var correctAnswer: String // 正解の答え
    var correctHiragana: String? // 正解のひらがな
    var exampleSentence: String? // 例文
    var exampleSentenceMeaning: String? // 例文の意味
}

class FavoriteViewControllerKR: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate {
    var mode: String = "kr_jp" // jp_kr または kr_jp
    ...
    func setupUI() {
        ...
        tableView.register(FavoriteTableViewCellKR.self, forCellReuseIdentifier: "FavoriteCellKR")
...
//            print("Error saving favorite questions: \(error)")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "FavoriteCellKR", for: indexPath) as? FavoriteTableViewCellKR else {
            fatalError("Failed to dequeue FavoriteTableViewCellKR.")
        }
...
class FavoriteTableViewCellKR: UITableViewCell {
    private let wordLabel = UILabel()
    private let meaningLabel = UILabel()
    private let exampleSentenceLabel = UILabel()
    private let exampleMeaningLabel = UILabel()
    private var wordAudioButton = UIButton(type: .system)
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            meaningLabel.bottomAnchor.constraint(lessThanOrEqualTo: contentView.bottomAnchor, constant: -10),
    ...
    @objc private func playWordAudio() {
        ...
        playAudio(for: word, language: "ko-KR") // 韓国語を設定
    }
