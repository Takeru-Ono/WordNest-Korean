...
class RapidMode_JP_ViewController: UIViewController, EndViewControllerDelegate {
    ...
    var language: String = "jp" // クイズ形式を指定 ("kr" or "jp")
...
//            print("Button \(index + 1): \(answer.0) - Tag: \(button.tag)") // デバッグ: 各ボタンのタイトルとタグを出力
    ...
    func configureButton(_ button: UIButton, with answer: (String, String, String)) {
        if #available(iOS 15.0, *) {
            // UIButtonConfigurationを使用する新しい方法
            var config = UIButton.Configuration.plain()
            config.title = answer.0
            config.image = UIImage(named: answer.2)?.resized(to: CGSize(width: 40, height: 40)) // 必要に応じて画像をリサイズ
            config.imagePlacement = .leading // アイコンを左に配置
            config.imagePadding = 10 // アイコンとタイトルの間隔
            config.baseForegroundColor = .black // テキストと画像の色
...
//        utterance.rate = 0.5  再生速度を調整
    ...
    func showEndScreen() {
        ...
        if let endVC = storyboard.instantiateViewController(withIdentifier: "EndViewController_JP_Rapid") as? EndViewController_JP_Rapid {
...
//            print("Error loading quiz results: \(error)")
    ...
    func getResultsFileURL() -> URL {
        ...
        return documentsURL.appendingPathComponent("quiz_results_Rapid_jp_kr.json")
    }
